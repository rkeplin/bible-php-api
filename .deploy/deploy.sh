...
ssh -t -oStrictHostKeyChecking=no -i $(pwd)/.deploy/travis_id_rsa travis@jersey1.rkeplin.com 'sed -i "s/image: '"$DOCKER_REPO_SLUG"':[a-zA-Z0-9]*/image: '"$DOCKER_REPO_SLUG"':'"$TRAVIS_BUILD_NUMBER"'/g" /opt/stacks/bible.yml'
